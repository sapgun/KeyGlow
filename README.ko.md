...
사용 중인 키보드와 가장 가까운 물리 배열을 고른 뒤, 화면에서 원하는 키를 직접 제어하면 됩니다.
...
KeyGlow는 물리 레이아웃과 동작 프로필을 별개로 관리합니다.

| 레이아웃 | 설명 |
| --- | --- |
| Full Size | 숫자패드가 포함된 표준 데스크톱 배열 |
| TKL | 숫자패드가 없는 데스크톱 배열 |
| 75% | F열과 방향키가 있는 컴팩트 배열 |
| 65% | 방향키는 있고 별도 F열은 없는 배열 |
| 60% | 최소 구성 컴팩트 배열 |

레이아웃은 `src/layouts/`의 데이터 파일로 관리되어, 렌더러를 다시 만들지 않고 새로운 배열을 추가할 수 있습니다.
...
Issue, 버그 리포트, 레이아웃 기여, UX 피드백, Pull Request를 환영합니다.
