...
使用中のキーボードに最も近い物理レイアウトを選び、画面上で各キーを直接コントロールできます。
...
KeyGlow は物理レイアウトと動作プロファイルを別々に管理します。

| レイアウト | 説明 |
| --- | --- |
| Full Size | テンキー付きの標準デスクトップ配列 |
| TKL | テンキーなしのデスクトップ配列 |
| 75% | F 列と矢印キーを備えたコンパクト配列 |
| 65% | 矢印キーあり、独立 F 列なし |
| 60% | 最小構成のコンパクト配列 |

レイアウトは `src/layouts/` のデータファイルとして管理されるため、レンダラーを作り直さずに新しい配列を追加できます。
...
Issue、バグ報告、レイアウト追加、UX フィードバック、Pull Request を歓迎します。
